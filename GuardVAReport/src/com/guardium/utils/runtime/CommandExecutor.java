...
public class CommandExecutor {
	 ...
	 * 
	 * In order to break the command line into its component arguments,
	 * you can either choose to run it in a bash shell, or you can 
	 * choose to use the built-in parser.  The built-in parser can handle
	 * single quotes, double quotes, and evaluates variable references that are not single-quoted as environment variables.
	 * 
	 * @param commandLine
	 * @param runInBashShell whether to use a bash shell to run the command
	 */
	public CommandExecutor(String commandLine, boolean runInBashShell) {
		if(runInBashShell) {
			this.parsedCommand = new String[] {"bash", "-c", commandLine};
		} else {
			this.parsedCommand = commandLinePattern.parse(commandLine);
		}
    ...
    public int handleProcess(Process process, Appendable stdOut, Appendable stdErr) throws IOException, InterruptedException {
    	return handleProcess(process, stdOut, stdErr, null);
    }
    
    private static StreamReader createReader(InputStream stream, Appendable sink, String charSet) throws UnsupportedEncodingException {
    	if(charSet == null) {
    ...
    private static OutputStreamWriter createWriter(OutputStream stream, String charSet) throws UnsupportedEncodingException {
    	if(charSet == null) {
    ...
    public int handleProcess(Process process, Appendable stdOut, Appendable stdErr, CharSequence stdIn) throws IOException, InterruptedException {
    ...
    private int handleProcess(Process process, StreamReader stdOutReader, StreamReader stdErrReader, CharSequence stdIn, OutputStreamWriter writer) throws IOException, InterruptedException, UnsupportedEncodingException {
    	if(writer != null) {
